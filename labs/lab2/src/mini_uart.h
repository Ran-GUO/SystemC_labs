...
SC_MODULE(MiniUart){
    ...
    addr("addr"), data_in("data_in"), rxd("rxd"),txd("txd"), 
    data_out("data_out"),irq_rx("irq_rx"), irq_tx("irq_tx"), 
    ClkUnit_inst("ClkUnit_inst"), TxUnit_inst("TxUnit_inst"), RxUnit_inst("RxUnit_inst"){
        ...
        SC_THREAD(interfaceProcess);
        //sensitive<<sys_clk.pos();
        SC_THREAD(combinationalProcess);
        //sensitive<<sys_clk.pos();
    }

    void interfaceProcess();
    void combinationalProcess();
